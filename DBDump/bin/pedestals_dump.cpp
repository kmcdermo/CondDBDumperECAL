...
namespace cond {
        class PedestalsDump : public Utilities {
                public:
...
cond::PedestalsDump::PedestalsDump():Utilities("cmscond_list_iov")
{
        addConnectOption();
...
cond::PedestalsDump::~PedestalsDump(){
}


void cond::PedestalsDump::dump(const A & ic, const char * filename)
{
        FILE * fd = fopen(filename, "w");
        if (!fd) {
                char err[256];
                sprintf(err, "[cond::PedestalsDump::dump] Impossible to open file `%s' for dumping:", filename);
                perror(err);
                exit(1);
        }
...
int cond::PedestalsDump::execute()
{

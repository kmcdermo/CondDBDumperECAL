...
namespace cond {
        class AlphaDump : public Utilities {
                public:
                        typedef EcalLaserAlphas A;
...
cond::AlphaDump::AlphaDump():Utilities("cmscond_list_iov")
{
        addConnectOption();
        ...
        addOption<std::string>("output","o","output file (default: alpha_dump_<tagname>_<IOV_start>_<IOV_stop>.dat)");
}

cond::AlphaDump::~AlphaDump(){
}


void cond::AlphaDump::dump(const A & alpha, const char * filename)
{
        FILE * fd = fopen(filename, "w");
        if (!fd) {
                char err[256];
                sprintf(err, "[cond::AlphaDump::dump] Impossible to open file `%s' for dumping:", filename);
                perror(err);
                exit(1);
        }
        ...
        for (size_t i = 0; i < ids.size(); ++i) {
                DetId id(ids[i]);
                ACit it = alpha.find(id);
                if (it == alpha.end()) {
                        fprintf(stderr, "Cannot find alpha for DetId %u", id.rawId());
                }
                if (id.subdetId() == EcalBarrel) {
                        EBDetId eid(id);
                        fprintf(fd, "%d %d %d %f\n", eid.ieta(), eid.iphi(), 0, *it);
                } else if (id.subdetId() == EcalEndcap) {
                        EEDetId eid(id);
                        fprintf(fd, "%d %d %d %f\n", eid.ix(), eid.iy(), eid.zside(), *it);
                } else {

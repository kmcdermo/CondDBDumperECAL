...
namespace cond {
        class ICDump : public Utilities {
                public:
                        typedef EcalTimeCalibConstants A;
...
cond::ICDump::ICDump():Utilities("cmscond_list_iov")
{
        addConnectOption();
...
void cond::ICDump::dump(const A & ic, const char * filename)
{
        ...
        for (size_t i = 0; i < ids.size(); ++i) {
                ...
                if (id.subdetId() == EcalBarrel) {
                        EBDetId eid(id);
                        fprintf(fd, "%d %d %d %f 0.01 %u\n", eid.ieta(), eid.iphi(), 0, *it, id.rawId());
                } else if (id.subdetId() == EcalEndcap) {
                        EEDetId eid(id);
                        fprintf(fd, "%d %d %d %f 0.01 %u\n", eid.ix(), eid.iy(), eid.zside(), *it, id.rawId());
                } else {

...
namespace cond {
        class ESICDump : public Utilities {
                public:
                        typedef ESIntercalibConstants A;
...
cond::ESICDump::ESICDump():Utilities("cmscond_list_iov")
{
        addConnectOption();
...
cond::ESICDump::~ESICDump(){
}


void cond::ESICDump::dump(const A & o, const char * filename)
{
        FILE * fd = fopen(filename, "w");
        if (!fd) {
                char err[256];
                sprintf(err, "[cond::ESICDump::dump] Impossible to open file `%s' for dumping:", filename);
                perror(err);
                exit(1);
        }

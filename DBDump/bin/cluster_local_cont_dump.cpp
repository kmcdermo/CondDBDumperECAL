...
namespace cond {
        class ClusterLocalContDump : public Utilities {
                public:
                        typedef EcalClusterLocalContCorrParameters A;
...
cond::ClusterLocalContDump::ClusterLocalContDump():Utilities("cmscond_list_iov")
{
        addConnectOption();
...
cond::ClusterLocalContDump::~ClusterLocalContDump(){
}


void cond::ClusterLocalContDump::dump(const A & a, const char * filename)
{
        FILE * fd = fopen(filename, "w");
        if (!fd) {
                char err[256];
                sprintf(err, "[cond::ClusterLocalContDump::dump] Impossible to open file `%s' for dumping:", filename);
                perror(err);
                exit(1);
        }
...
int cond::ClusterLocalContDump::execute()
{
...
{
        cond::ClusterLocalContDump dump;

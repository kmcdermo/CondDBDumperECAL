...
namespace cond {
        class PedDump : public Utilities {
                public:
                        typedef EcalPedestals A;
...
cond::PedDump::PedDump():Utilities("cmscond_list_iov")
{
        addConnectOption();
...
cond::PedDump::~PedDump(){
}


void cond::PedDump::dump(const A & ic, const char * filename)
{
        FILE * fd = fopen(filename, "w");
        if (!fd) {
                char err[256];
                sprintf(err, "[cond::PedDump::dump] Impossible to open file `%s' for dumping:", filename);
                perror(err);
                exit(1);
        }
        ...
        fprintf(fd, "#ieta/ix iphi/iy 0/izeta  g12 g12_rms  g6 g6_rms  g1 g1_rms  rawId\n");

...
namespace cond {
        class TPGLinearizationConstDump : public Utilities {
                public:
                        typedef EcalTPGLinearizationConst A;
...
cond::TPGLinearizationConstDump::TPGLinearizationConstDump():Utilities("cmscond_list_iov")
{
        addConnectOption();
...
cond::TPGLinearizationConstDump::~TPGLinearizationConstDump(){
}


void cond::TPGLinearizationConstDump::dump(const A & ic, const char * filename)
{
        FILE * fd = fopen(filename, "w");
        if (!fd) {
                char err[256];
                sprintf(err, "[cond::TPGLinearizationConstDump::dump] Impossible to open file `%s' for dumping:", filename);
                perror(err);
                exit(1);
        }
...
int cond::TPGLinearizationConstDump::execute()
{
...
{
        cond::TPGLinearizationConstDump dump;

...
namespace cond {
        class TimeOffsetDump : public Utilities {
                public:
                        typedef EcalTimeOffsetConstant A;
...
cond::TimeOffsetDump::TimeOffsetDump():Utilities("cmscond_list_iov")
{
        addConnectOption();
...
cond::TimeOffsetDump::~TimeOffsetDump(){
}


void cond::TimeOffsetDump::dump(const A & ag, const char * filename)
{
        FILE * fd = fopen(filename, "w");
        if (!fd) {
                char err[256];
                sprintf(err, "[cond::TimeOffsetDump::dump] Impossible to open file `%s' for dumping:", filename);
                perror(err);
                exit(1);
        }
...
int cond::TimeOffsetDump::execute()
{
